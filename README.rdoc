...
== how it works
 ...
 1. catch `request.fullpath`
 2. match user-defined rule based on current route's `controller#action` and `request.GET` params
 3. match user-defined pattern for every meta tag based on matched rule
 4. compose every meta tag using user-defined static and dynamic snippets based on matched pattern
